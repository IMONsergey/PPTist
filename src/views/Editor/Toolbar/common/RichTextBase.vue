...
const updateLink = (link?: string) => {
  ...
  if (!link || !linkRegExp.test(link)) return message.error('Некорректный адрес веб-ссылки')
...
const execAI = async (command: string) => {
  ...
  if (!content) return message.error('Нет текста для обработки')
