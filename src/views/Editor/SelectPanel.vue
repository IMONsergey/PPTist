<template>
  ...
  >
    <div class="handler" v-if="elements.length">
      <div class="btns">
        <Button size="small" style="margin-right: 5px;" @click="showAllElements()">Показать все</Button>
        <Button size="small" @click="hideAllElements()">Скрыть все</Button>
      </div>
    ...
    <div class="element-list" v-if="elements.length">
      <template v-for="item in elements" :key="item.id">
        <div class="group-els" v-if="item.type === 'group'">
          <div class="group-title">Группа</div>
          <div
            class="item"
            :class="{
