<template>
  <div class="media-input">
    ...
    <template v-if="type === 'video'">
      <Input v-model:value="videoSrc" placeholder="Введите URL видео, напр. https://xxx.mp4"></Input>
      <div class="btns">
        <FileInput accept="video/*" @change="files => uploadVideo(files)">
          <Button><i-icon-park-outline:upload /> Загрузить локальное видео</Button>
        </FileInput>
        <div class="group">
          <Button @click="emit('close')" style="margin-right: 10px;">Отмена</Button>
          <Button type="primary" @click="insertVideo()">Подтвердить</Button>
        </div>
      </div>
    </template>

    <template v-if="type === 'audio'">
      <Input v-model:value="audioSrc" placeholder="Введите URL аудио, напр. https://xxx.mp3"></Input>
      <div class="btns">
...
const insertVideo = () => {
  if (!videoSrc.value) return message.error('Пожалуйста, введите корректный URL видео')
  emit('insertVideo', { src: videoSrc.value })
}

const insertAudio = () => {
  if (!audioSrc.value) return message.error('Пожалуйста, введите корректный URL аудио')
